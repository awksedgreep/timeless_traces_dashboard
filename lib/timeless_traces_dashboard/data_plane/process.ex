defmodule TimelessTracesDashboard.DataPlane.Process do
  ...
  def handle_info({port, {:exit_status, status}}, %{port: port} = state) do
    {:stop, {:traces_data_plane_exit, status}, state}
  end

  def handle_info({:EXIT, port, reason}, %{port: port} = state) do
    {:stop, {:traces_data_plane_port_exit, reason}, state}
  end
  ...
  defp validate_options(opts) do
    with {:ok, binary} <- required_path(opts, :binary),
         ...
         {:ok, listen} <- loopback_listener(Keyword.get(opts, :listen, "127.0.0.1:19449")),
  ...
  defp required_path(opts, name) do
    with {:ok, path} <- required_option(opts, name) do
      ...
      if File.regular?(path) do
        ...
        {:error, {:invalid_traces_data_plane_path, name, path}}
      end
    end
  end
  ...
  defp required_option(opts, name) do
    case Keyword.fetch(opts, name) do
      ...
      _ -> {:error, {:missing_traces_data_plane_option, name}}
    end
  end
  ...
  defp loopback_listener(listen) when is_binary(listen) do
    with [host, port_text] <- String.split(listen, ":", parts: 2),
      ...
      _ -> {:error, {:traces_data_plane_must_use_loopback, listen}}
    end
  end

  defp loopback_listener(listen), do: {:error, {:traces_data_plane_must_use_loopback, listen}}
  ...
  defp shutdown_executable do
    case System.find_executable("kill") do
      nil -> {:error, :traces_data_plane_requires_kill_executable}
  ...
  defp open_port(config) do
    ...
    error in [ArgumentError, ErlangError] -> {:error, {:start_traces_data_plane, error}}
  end
